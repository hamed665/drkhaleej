...
- Import-readiness runtime is aligned through **PR #974** at baseline **`e3b759a3e0b07a7458addab416a44917f3e43801`**.
- Database/migration status: **validates through `0094_import_entity_resolution_gate.sql`**.
- Completed migration set: **`0001` through `0094`**.
- Current import-readiness implementation: **`AUTOMATION-JOB-RUNTIME`** is the next independently reviewable PR. The completed documentation-only gate is recorded in [`docs/import/WORKER_RUNTIME_ADR_GATE.md`](docs/import/WORKER_RUNTIME_ADR_GATE.md), with accepted concrete choices in [`docs/import/WORKER_RUNTIME_ARCHITECTURE_DECISION.md`](docs/import/WORKER_RUNTIME_ARCHITECTURE_DECISION.md). Naming P19 does not implement or activate it; decision application and every downstream runtime/Production authority remain gated.
