...
export async function listAdminMediaAssets(filters: AdminMediaFilters): Promise<{ ok: true; items: AdminMediaAsset[] } | { ok: false }> {
  ...
  try {
    ...
    let query = supabase
      .from("media_assets")
      .select("id, original_filename, storage_bucket, storage_path, mime_type, file_size_bytes, width, height, alt_text_en, alt_text_ar, caption_en, caption_ar, admin_usage_kind, admin_review_status, status, admin_visibility_status, is_archived, created_at, updated_at, entity_media(count)")
