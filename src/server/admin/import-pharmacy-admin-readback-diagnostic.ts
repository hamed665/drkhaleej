...
export async function diagnosePharmacyAdminReadback(input: {
  ...
  if (actorIds[0] !== input.actorId) return "actor_allowlist_mismatch";
  ...
  try {
    const center = await client
      ...
      .maybeSingle();
    if (center.error) return "centers_read_failed";
    ...
    const readStates = await client
      ...
      .limit(1);
    if (readStates.error) return "read_states_read_failed";
    ...
    const authorizations = await client
      ...
      .eq("entity_id", input.entityId)
      .limit(1);
    if (authorizations.error) return "authorizations_read_failed";
    ...
    const reservations = await client
      ...
      .eq("entity_id", input.entityId)
      .limit(1);
    if (reservations.error) return "reservations_read_failed";

    return "ready";
  } catch {
    return "centers_read_failed";
  }
}
