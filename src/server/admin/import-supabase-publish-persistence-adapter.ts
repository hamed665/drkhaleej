...
export type ImportSupabaseRpcError = { code?: string; message?: string };
export type ImportSupabaseRpcResponse = { data: unknown; error: ImportSupabaseRpcError | null };
...
function durationUnits(expiresAt: string, unitMs: number, min: number, max: number): number | null {
  ...
  return Number.isInteger(units) && units >= min && units <= max ? units : null;
}
function normalizeTerminalResult(value: unknown): ImportPublishPersistenceTerminalResult | null {
  ...
  const requiredStrings = [value.entityId, value.idempotencyKey, value.requestHash, value.auditEventId, value.rollbackSnapshotId];
...
function normalizeReservationResponse(data: unknown): ImportPublishPersistenceTransactionResult {
  if (!isRecord(data) || typeof data.status !== "string") return { kind: "failed", reason: "rpc_failed" };
  if (data.status === "reserved") {
    if (typeof data.idempotencyRecordId !== "string" || typeof data.rollbackSnapshotId !== "string" || typeof data.auditEventId !== "string") {
  ...
  if (data.status === "replayed") {
    ...
    return terminalResult ? { kind: "replayed", terminalResult } : { kind: "failed", reason: "rpc_failed" };
  }
  if (data.status === "conflict") {
    ...
    if (
      ...
      reason === "reservation_replay_incomplete"
    ) return { kind: "conflict", reason };
  }
  return { kind: "failed", reason: "rpc_failed" };
}
function normalizeTerminalResponse(data: unknown, request: ImportPublishPersistenceTerminalWriteRequest): ImportPublishPersistenceTerminalWriteResult {
  if (!isRecord(data) || typeof data.status !== "string") return { kind: "failed", reason: "rpc_failed" };
  if (data.status === "replayed") {
    const terminalResult = normalizeTerminalResult(data.terminalResult);
    return terminalResult ? { kind: "replayed", terminalResult } : { kind: "failed", reason: "rpc_failed" };
  }
  if (data.status === "conflict" && data.reason === "terminal_identity_mismatch") return { kind: "conflict", reason: "terminal_identity_mismatch" };
  if (data.status === "failed") {
    if (data.reason === "idempotency_record_not_found" || data.reason === "rollback_snapshot_not_found") return { kind: "failed", reason: data.reason };
    return { kind: "failed", reason: "rpc_failed" };
  }
  if ((data.status === "succeeded" || data.status === "failed" || data.status === "rolled_back") && typeof data.idempotencyRecordId === "string" && typeof data.auditEventId === "string") {
    return { kind: "persisted", reservationId: data.idempotencyRecordId, auditEventId: data.auditEventId, outcome: request.outcome };
  }
  return { kind: "failed", reason: "rpc_failed" };
}

export function createImportSupabasePublishPersistenceAdapter(client: ImportSupabaseRpcClient): ImportPrivatePublishPersistenceAdapter {
  return {
    async runReservationSnapshotAuditTransaction(request) {
    ...
    async persistTerminalResult(request) {
