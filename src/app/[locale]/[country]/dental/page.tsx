...
const metadataCopyByLocale: Record<SupportedLocale, { title: string; description: string }> = {
  en: {
    ...
    description: 'Browse dental clinics, dentists, orthodontics, implants, whitening and oral care services in Oman. Public discovery only, not medical advice.'
  },
  ar: {
    title: 'عيادات الأسنان في عُمان | DrMuscat',
    description: 'تصفح عيادات الأسنان وأطباء الأسنان والتقويم والزراعة والتبييض وخدمات العناية بالفم في عُمان. اكتشاف عام فقط وليس نصيحة طبية.'
  }
};
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return buildLocalizedMetadata({ locale, country, pathname: '/dental', title: copy.title, description: copy.description });
}

export default async function PublicDentalPage({ params }: { params: Promise<Params> }) {
  const { locale, country } = await params;
  ...
  return (
    <main className="home-foundation dm2026-home-page dm2026-doctors-page dm2026-public-discovery-page dm2026-public-discovery-page--dental" dir={dir} data-country={safeCountry} data-locale={safeLocale}>
