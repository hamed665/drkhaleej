...
for (const token of ['.update(', '.delete(', '.insert(', 'revalidatePath', 'writeAdminAuditEvent']) {
...
for (const token of ['deactivateActiveCenterPublicProfile', 'useActionState', '<form', 'type="submit"', 'revalidatePath']) {
...
for (const token of ['deactivateActiveCenterPublicProfile', 'useActionState', '<form', 'action={', 'type="submit"']) {
...
for (const token of ['label: "Active Centers"', '"active_centers.public_state.update"']) {
