...
const sitemapSchemaVersion = 'drkhaleej.import.pharmacySitemapPromotion.v1';
const evidencePath =
  process.env.PHARMACY_SITEMAP_EVIDENCE_PATH ??
  'artifacts/pharmacy-sitemap-promotion/proof.json';
...
function verifyPreviewIdentity(databaseUrl, previewRef, productionRef) {
  ...
  assert(parsed.protocol === 'postgresql:', 'Sitemap proof requires postgresql.');
  assert(parsed.port === '5432', 'Sitemap proof requires Session pooler port 5432.');
  assert(
    parsed.hostname.endsWith('.pooler.supabase.com'),
    'Sitemap proof requires the isolated Supabase Session pooler.',
  );
  assert(
...
function connectionConfig(databaseUrl, suffix) {
  return {
    ...
    application_name: `drkhaleej-pharmacy-sitemap-${suffix}`,
...
function fixture(runId) {
  const seed = `pharmacy-sitemap-promotion:${runId}`;
  const slug = `sitemap-proof-${digest(seed).slice(0, 12)}`;
  return {
    ...
    sitemapIdempotencyKey: `sitemap-${digest(`${seed}:sitemap-key`).slice(0, 40)}`,
    sitemapRequestHash: digest(`${seed}:sitemap-request`),
    candidatePayload: {
      projectionVersion: 'v1',
      identity: {
        primaryName: 'Preview Sitemap Proof Pharmacy',
        nameEn: 'Preview Sitemap Proof Pharmacy',
      ...
      languages: ['en', 'ar'],
      source: {
        sourceName: 'Isolated Preview P15 proof',
...
async function verifyMigration(client) {
  ...
  const rpcs = await client.query(
    `select
     ...
     where p.oid in (
       to_regprocedure(
         'public.import_authorize_pharmacy_sitemap_promotion(uuid,uuid,text,text,text,integer)'
       ),
       to_regprocedure(
         'public.import_include_pharmacy_sitemap_by_authority(uuid,uuid,uuid,text,text)'
       ),
       to_regprocedure(
         'public.import_rollback_pharmacy_sitemap_by_authority(uuid,uuid,text)'
       )
     )
     order by signature`,
  );
  assert(
    ...
    'P15 RPC privilege boundary drifted.',
  );
  ...
  assert(
    ...
    'P15 authority tables must keep RLS and zero public policies.',
  );
}
...
async function cleanup(client, item) {
  ...
  try {
    await client.query(
      'delete from public.import_pharmacy_sitemap_events where entity_id = $1',
      [item.entityId],
    );
    await client.query(
      'delete from public.import_pharmacy_sitemap_authorizations where entity_id = $1',
      [item.entityId],
    );
    await client.query(
...
async function promoteIndex(client, item) {
  const result = await client.query(
...
async function provePromotionTamperFailsClosed(client, item) {
  ...
  const blocked = await promoteIndex(client, item);
  assert(
...
async function authorizeSitemap(client, item) {
  const result = await client.query(
    `select public.import_authorize_pharmacy_sitemap_promotion(
       $1::uuid, $2::uuid, $3::text, $4::text, $5::text, $6::integer
     ) as result`,
    [
      item.actorId,
      item.entityId,
      item.sitemapIdempotencyKey,
      item.sitemapRequestHash,
      sitemapSchemaVersion,
      24,
    ],
  );
  const authority = result.rows[0]?.result;
  assert(authority?.status === 'issued', 'P15 Sitemap authority was not issued.');
  item.sitemapAuthorizationId = authority.authorizationId;
}

async function includeSitemap(client, item) {
  const result = await client.query(
    `select public.import_include_pharmacy_sitemap_by_authority(
       $1::uuid, $2::uuid, $3::uuid, $4::text, $5::text
     ) as result`,
    [
      item.sitemapAuthorizationId,
      item.actorId,
      item.entityId,
      item.sitemapRequestHash,
      sitemapSchemaVersion,
...
async function rollbackSitemap(client, item) {
  const result = await client.query(
    `select public.import_rollback_pharmacy_sitemap_by_authority(
       $1::uuid, $2::uuid, $3::text
     ) as result`,
    [item.actorId, item.entityId, sitemapSchemaVersion],
  );
  return result.rows[0]?.result;
}

async function proveSitemapAuthorizationTamperFailsClosed(client, item) {
  const queue = await readQueue(client, item);
  await client.query(
    `update public.import_publish_queue
     set metadata = metadata || '{"sitemap_included":true}'::jsonb
  ...
  assert(
    blocked.rows[0]?.result?.status === 'conflict' &&
      blocked.rows[0]?.result?.reason === 'sitemap_prerequisite_queue_integrity_mismatch' &&
      blocked.rows[0]?.result?.authorityConsumed === false,
    'P14 Queue tamper did not block P15 authorization.',
  );
  const writes = await client.query(
    `select
       (select count(*)::int from public.import_pharmacy_sitemap_authorizations where entity_id = $1) as authorities,
       (select count(*)::int from public.import_pharmacy_sitemap_events where entity_id = $1) as events`,
    [item.entityId],
  );
  assert(
    writes.rows[0]?.authorities === 0 && writes.rows[0]?.events === 0,
    'Blocked P15 authorization persisted authority or event state.',
  );
  await client.query(
    'update public.import_publish_queue set metadata = $2::jsonb where id = $1',
    [queue.id, JSON.stringify(queue.metadata)],
...
async function proveSitemapInclusionTamperFailsClosed(client, item) {
  const queue = await readQueue(client, item);
  await client.query(
    `update public.import_publish_queue
     set metadata = metadata || '{"sitemap_included":true}'::jsonb
  ...
  const blocked = await includeSitemap(client, item);
  assert(
    blocked?.status === 'conflict' &&
      blocked?.reason === 'sitemap_prerequisite_queue_integrity_mismatch' &&
      blocked?.authorityConsumed === false,
    'Pre-inclusion Queue tamper did not fail P15 closed.',
  );
  const state = await client.query(
    `select
       a.status,
       count(e.id) filter (where e.event_type = 'sitemap_included')::int as inclusion_events
     from public.import_pharmacy_sitemap_authorizations a
     left join public.import_pharmacy_sitemap_events e on e.authorization_id = a.id
     where a.id = $1
     group by a.status`,
    [item.sitemapAuthorizationId],
  );
  assert(
    state.rows[0]?.status === 'issued' && state.rows[0]?.inclusion_events === 0,
    'P15 inclusion tamper consumed authority or wrote an event.',
  );
  await client.query(
    'update public.import_publish_queue set metadata = $2::jsonb where id = $1',
    [queue.id, JSON.stringify(queue.metadata)],
...
async function verifySitemapIncluded(client, item) {
  const queue = await readQueue(client, item);
  assert(
    queue.publish_status === 'index_eligible' &&
      queue.index_policy === 'index' &&
      queue.sitemap_policy === 'included' &&
      queue.metadata?.robots_policy === 'index' &&
      queue.metadata?.index_promoted === true &&
      queue.metadata?.sitemap_included === true &&
      queue.metadata?.pharmacy_index_promotion_schema_version === indexSchemaVersion &&
      queue.metadata?.pharmacy_index_authorization_id === item.indexAuthorizationId &&
      queue.metadata?.pharmacy_sitemap_promotion_schema_version === sitemapSchemaVersion &&
      queue.metadata?.pharmacy_sitemap_authorization_id === item.sitemapAuthorizationId &&
      queue.metadata?.canonical_path === item.canonicalPathEn &&
      queue.metadata?.canonical_paths?.ar === item.canonicalPathAr,
    'Included Queue readback crossed or missed the P15 boundary.',
  );
  ...
  const state = await client.query(
    `select
       ...
       a.snapshot_payload,
       a.snapshot_hash,
       encode(extensions.digest(a.snapshot_payload::text, 'sha256'), 'hex') as computed_snapshot_hash,
       a.terminal_result,
       a.terminal_result_hash,
       encode(extensions.digest(a.terminal_result::text, 'sha256'), 'hex') as computed_terminal_hash,
       i.status as index_status,
       c.status::text as center_status,
       c.is_active,
       count(e.id) filter (where e.event_type = 'sitemap_included')::int as inclusion_events
     from public.import_pharmacy_sitemap_authorizations a
     join public.import_pharmacy_index_authorizations i on i.id = a.index_authorization_id
     join public.centers c on c.id = a.entity_id
     left join public.import_pharmacy_sitemap_events e on e.authorization_id = a.id
     where a.id = $1
     group by a.id, i.status, c.status, c.is_active`,
    [item.sitemapAuthorizationId],
  );
  const row = state.rows[0];
  assert(
    row?.status === 'included' &&
      row.index_status === 'promoted' &&
      row.center_status === 'draft' &&
      row.is_active === false &&
      row.inclusion_events === 1 &&
      row.computed_snapshot_hash === row.snapshot_hash &&
      row.computed_terminal_hash === row.terminal_result_hash &&
      row.terminal_result?.kind === 'pharmacy_sitemap_included' &&
      row.terminal_result?.indexPolicy === 'index' &&
      row.terminal_result?.sitemapPolicy === 'included' &&
      row.terminal_result?.sitemapIncluded === true,
    'P15 included authority readback drifted.',
  );
}

async function proveSitemapRollbackTamperFailsClosed(client, item) {
  const queue = await readQueue(client, item);
  await client.query(
    `update public.import_publish_queue
     set metadata = metadata || '{"sitemap_included":false}'::jsonb
     where id = $1`,
    [queue.id],
  );
  const blocked = await rollbackSitemap(client, item);
  assert(
    blocked?.status === 'conflict' &&
      blocked?.reason === 'sitemap_included_queue_integrity_mismatch' &&
      blocked?.authorityConsumed === false,
    'Included Queue tamper did not fail P15 rollback closed.',
  );
  const state = await client.query(
    `select
       a.status,
       count(e.id) filter (where e.event_type = 'sitemap_rolled_back')::int as rollback_events
     from public.import_pharmacy_sitemap_authorizations a
     left join public.import_pharmacy_sitemap_events e on e.authorization_id = a.id
     where a.id = $1
     group by a.status`,
  ...
  assert(
    state.rows[0]?.status === 'included' && state.rows[0]?.rollback_events === 0,
    'P15 rollback tamper consumed authority or wrote an event.',
  );
  await client.query(
    'update public.import_publish_queue set metadata = $2::jsonb where id = $1',
    [queue.id, JSON.stringify(queue.metadata)],
...
async function verifySitemapRolledBack(client, item) {
  const result = await client.query(
    `select
       a.status,
       a.snapshot_payload,
  ...
  assert(
    row?.status === 'rolled_back' &&
      ...
      row.computed_snapshot_hash === row.snapshot_hash &&
      row.computed_terminal_hash === row.terminal_result_hash &&
      row.terminal_result?.visibility === 'public' &&
      row.terminal_result?.indexPolicy === 'index_eligible' &&
      row.terminal_result?.sitemapPolicy === 'excluded' &&
      row.terminal_result?.exactLogicalRecovery === true,
    'Rolled-back P15 authority readback drifted.',
  );
  ...
  assert(
    queue.id === snapshot?.id &&
      ...
      queue.metadata?.index_promoted === true &&
      queue.metadata?.sitemap_included === false &&
      queue.metadata?.pharmacy_sitemap_promotion_schema_version === undefined,
    'P15 rollback did not restore the exact P14 Index Queue.',
  );
}
...
async function verifyCleanup(client, item) {
  const result = await client.query(
    `select
       (select count(*)::int from public.import_pharmacy_sitemap_events where entity_id = $1) as sitemap_events,
       (select count(*)::int from public.import_pharmacy_sitemap_authorizations where entity_id = $1) as sitemap_authorities,
  ...
  assert(
    ...
    'P15 cleanup left fixture rows.',
  );
}

const databaseUrl = required('PHARMACY_SITEMAP_PREVIEW_DATABASE_URL');
const previewRef = required('PHARMACY_SITEMAP_PREVIEW_PROJECT_REF');
const productionRef = required('PHARMACY_SITEMAP_PRODUCTION_PROJECT_REF');
const sourceCommit = required('PHARMACY_SITEMAP_SOURCE_COMMIT');
const runId = required('PHARMACY_SITEMAP_RUN_ID');
...
try {
  ...
  const promotionResults = await Promise.all([
    promoteIndex(clientA, item),
    promoteIndex(clientB, item),
  ]);
  assert(
    promotionResults
  ...
  await proveSitemapAuthorizationTamperFailsClosed(clientA, item);
  await authorizeSitemap(clientA, item);
  await proveSitemapInclusionTamperFailsClosed(clientA, item);

  const inclusionResults = await Promise.all([
    includeSitemap(clientA, item),
    includeSitemap(clientB, item),
  ]);
  assert(
    inclusionResults
      .map((result) => result?.status)
      .sort()
      .join(',') === 'included,replayed',
    'Concurrent P15 inclusion must produce one inclusion and one replay.',
  );
  await verifySitemapIncluded(clientA, item);
  await proveSitemapRollbackTamperFailsClosed(clientA, item);

  const rollbackResults = await Promise.all([
    rollbackSitemap(clientA, item),
    rollbackSitemap(clientB, item),
  ]);
  assert(
    rollbackResults
      .map((result) => result?.status)
    ...
    'Concurrent P15 rollback must produce one rollback and one replay.',
  );
  const persistedReplay = await rollbackSitemap(clientA, item);
  assert(
    persistedReplay?.status === 'replayed' &&
      persistedReplay?.exactLogicalRecovery === true &&
      persistedReplay?.indexPolicy === 'index_eligible' &&
      persistedReplay?.sitemapIncluded === false,
    'P15 persisted rollback replay did not use exact P14 readback.',
  );
  await verifySitemapRolledBack(clientA, item);
  await cleanup(clientA, item);
  await verifyCleanup(clientA, item);
  verified = true;

  await writeEvidence({
    schemaVersion: 'drkhaleej.pharmacySitemapPromotionProof.v1',
    status: 'green',
    sourceCommit,
    ...
    p14IndexPrerequisiteVerified: true,
    p14TamperBlockedBeforeConsumption: true,
    sitemapAuthorizationTamperBlockedWithoutWrites: true,
    sitemapInclusionTamperBlockedBeforeConsumption: true,
    concurrentInclusionCount: 2,
    freshInclusionCount: 1,
    concurrentInclusionReplayCount: 1,
    includedReadbackVerified: true,
    robotsPolicyAfterInclusion: 'index',
    sitemapIncludedAfterInclusion: true,
    ...
    sitemapEventCountPerAuthority: 3,
  ...
  console.error(
    `Pharmacy Sitemap promotion proof failed: ${redact(
  ...
  if (verified) {
    console.log('Pharmacy Sitemap promotion proof passed on isolated Preview.');
  }
}
