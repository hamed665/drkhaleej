...
- Migration validation is expected through `0095_import_automation_job_runtime.sql`.
...
- Database foundations validate through `0095_import_automation_job_runtime.sql`.
...
- A fail-closed Preview Migration Sync workflow validates repository migration/RLS contracts, verifies isolated Preview identity, applies ledger-missing migrations under an advisory lock, verifies the exact ledger, runs the existing import regressions through Entity Resolution, and proves P19 request replay, concurrent claim, expired-lease reclaim, stale-worker fencing, bounded artifacts, atomic completion/outbox and cleanup. Production is not connected.
