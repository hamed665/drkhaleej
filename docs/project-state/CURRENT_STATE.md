...
- Migration validation is expected through `0087_import_pharmacy_public_noindex_lifecycle.sql`.
...
| Current migration | `0087_import_pharmacy_public_noindex_lifecycle.sql` |
...
| 7.1 | COMPLETE | Pharmacy public/noindex authority, bilingual route, exact readback and rollback proven; Index and Sitemap remain closed |

The Admin reservation operation and bounded authorization-linked integrity readback are implemented and proven on an isolated Preview database. P03 additionally proved replay, conflict, two-client lock waiting, forced rollback at all four write boundaries, deterministic cleanup and zero partial writes. P04-A writes `reservation_created` with the exact v2 schema while retaining legacy reader compatibility. P04-B hands only an already verified, fully bound Reservation to an injected server-only executor port and invokes no second Reservation. P05 wires that handoff to the existing Pharmacy mutation authority, appends mutation-phase `execution_started` v3, applies the exact reviewed canonical patch, persists terminal success, creates one server-only durable rollback reference and requires post-mutation readback. Rollback authority hardening is complete. P06 and P07 prove server-selected rollback, bounded replay and exact logical recovery. P08/P09 complete the ten-stage Admin state machine and fixed isolated Preview cycle with integrity-zero readback. Registry Authority Audit and Registry Convergence establish the total fail-closed family and route adapter. The Pharmacy public/noindex lifecycle then adds one separate single-entity authority and exact Queue snapshot, two canonical localized routes with reciprocal metadata, family-correct structured data, forced `noindex`, Sitemap exclusion, concurrency-bounded publish/rollback and exact recovery. Its record is [`docs/import/PHARMACY_PUBLIC_NOINDEX_LIFECYCLE.md`](../import/PHARMACY_PUBLIC_NOINDEX_LIFECYCLE.md). `PHARMACY-INDEX-PROMOTION` is the next gate. Sitemap promotion, Production execution, Agent, Content, Hospital, Doctor, later families and Bulk remain disabled. AI-assisted intake and the Content/SEO Agent are planned capabilities, not production implementations.
...
- Database foundations validate through `0087_import_pharmacy_public_noindex_lifecycle.sql`.
...
- Migration validation passes through `0087_import_pharmacy_public_noindex_lifecycle.sql`.
