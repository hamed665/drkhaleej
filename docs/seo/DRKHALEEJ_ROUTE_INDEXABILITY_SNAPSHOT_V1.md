...
It is documentation and validation support only. It does not approve new routes, database publishing, ads rendering, offers rendering, public comments, ratings, appointment workflows, payments, or provider dashboard work.
...
| `/hospitals` | `/en/om/hospitals`, `/ar/om/hospitals` | `index` | `ready` | included | default indexable | index-ready | Directory page stays static; imported hospital detail pages remain blocked. |
...
| Imported hospital profile | `/en/om/hospitals/{slug}` and `/ar/om/hospitals/{slug}` | `PROFILE-GATE-D imported-hospital-profile-guard-v1` is active, but public hospital detail route and API are blocked | allowed only through guarded import sitemap rows | sitemap eligibility requires queue metadata `index`, `included`, exact canonical match, approved candidate, source evidence, contact/map evidence, and Oman geo evidence; public detail rendering remains blocked | sitemap eligibility only, no public detail route | Keep hospital detail/discovery on hold until a dedicated projection release PR. |
...
The smoke check keeps doctor and pharmacy public routes aligned and keeps hospital detail routes blocked while hospital sitemap eligibility remains gate-controlled.
...
- guarded imported hospital sitemap URLs only when `import_publish_queue` says `index`, `included`, `sitemap_included: true`, `robots_policy: index`, the canonical path is exact, the candidate is approved, and required evidence exists; public hospital detail rendering remains blocked.
...
- Public hospital detail and discovery routes.
- Public comments, ratings, appointment workflows, payment, and provider dashboard routes.

## First launch decision

The next implementation step should be a real import batch rehearsal using `IMPORT-QA-A`, `SITEMAP-GUARD-B`, and `PROFILE-SMOKE-A` together before any broad provider import goes public.
