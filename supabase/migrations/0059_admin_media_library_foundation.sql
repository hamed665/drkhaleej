...
ALTER TABLE public.media_assets
  ...
  ADD COLUMN IF NOT EXISTS admin_review_status text NOT NULL DEFAULT 'draft',
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'media_assets_admin_review_status_check' AND conrelid = 'public.media_assets'::regclass) THEN
    ALTER TABLE public.media_assets
      ADD CONSTRAINT media_assets_admin_review_status_check
      CHECK (admin_review_status IN ('draft', 'reviewing', 'approved', 'rejected'));
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'media_assets_admin_visibility_status_check' AND conrelid = 'public.media_assets'::regclass) THEN
    ALTER TABLE public.media_assets
      ...
      CHECK (admin_visibility_status IN ('private', 'public_candidate'));
  END IF;
...
CREATE INDEX IF NOT EXISTS media_assets_admin_review_status_idx ON public.media_assets(admin_review_status);
