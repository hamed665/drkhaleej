-- P09 REAL-ADMIN-CANARY: accept equivalent timestamptz wire formats at the private publish boundary.
-- Browser/PostgREST reads use ISO-8601 while PostgreSQL timestamptz::text uses a database display format.
-- The version authority remains exact, but equality is temporal rather than presentation-string equality.
...
begin
  ...
  begin
    v_expected_version := p_expected_version::timestamptz;
  ...
  if v_center.updated_at is distinct from v_expected_version then
    return jsonb_build_object('status', 'conflict', 'reason', 'expected_version_mismatch');
  end if;
...
comment on function public.import_publish_pharmacy_private(
  uuid, uuid, uuid, uuid, uuid, text, jsonb, text
) is 'Atomic service-role-only Pharmacy private mutation using an already verified Reservation. Expected entity versions are compared as timestamptz values so equivalent ISO/PostgreSQL wire formats remain exact.';
