...
--
-- Purpose:
-- - Insert/update the official base commercial plan catalog used by admin flows.
...
UPDATE public.subscription_plans AS existing
SET
  name_en = plan_rows.name_en,
  name_ar = plan_rows.name_ar,
  description_en = plan_rows.description_en,
  description_ar = plan_rows.description_ar,
  price_amount = plan_rows.price_amount,
  currency_code = plan_rows.currency_code,
  interval = plan_rows.interval,
  status = plan_rows.status,
  includes_claim_badge = plan_rows.includes_claim_badge,
  includes_featured_listing = plan_rows.includes_featured_listing,
  includes_media_gallery = plan_rows.includes_media_gallery,
  max_doctors = plan_rows.max_doctors,
  max_locations = plan_rows.max_locations,
  max_services = plan_rows.max_services,
  sort_order = plan_rows.sort_order,
  metadata = plan_rows.metadata,
  deleted_at = NULL,
  updated_at = now()
FROM plan_rows
WHERE existing.slug = plan_rows.slug;
...
SELECT
  ...
  plan_rows.name_en,
  plan_rows.name_ar,
  plan_rows.description_en,
  plan_rows.description_ar,
  plan_rows.price_amount,
  plan_rows.currency_code,
  plan_rows.interval,
  plan_rows.status,
  plan_rows.includes_claim_badge,
  plan_rows.includes_featured_listing,
  plan_rows.includes_media_gallery,
  plan_rows.max_doctors,
  plan_rows.max_locations,
  plan_rows.max_services,
  plan_rows.sort_order,
  plan_rows.metadata
FROM plan_rows
WHERE NOT EXISTS (
  SELECT 1
  FROM public.subscription_plans AS existing
  WHERE existing.slug = plan_rows.slug
);
