...
-- Independent, single-entity Pharmacy public/noindex authority and Queue snapshot.
-- Public routes and rollback remain disabled for later independently reviewed PRs.
...
create table if not exists public.import_pharmacy_public_noindex_authorizations (
  ...
  constraint import_pharmacy_public_noindex_status_check
    check (status in ('issued', 'published', 'invalidated', 'expired')),
...
create table if not exists public.import_pharmacy_public_noindex_events (
  ...
  constraint import_pharmacy_public_noindex_event_type_check
    check (event_type in ('authorization_issued', 'public_noindex_published')),
  constraint import_pharmacy_public_noindex_event_outcome_check
    check (outcome in ('issued', 'published')),
...
begin
  ...
  if found then
    ...
    set target_entity_id = p_entity_id,
        ...
        admin_note = 'Public Pharmacy noindex authority; routes and rollback remain separately locked.',
    ...
    insert into public.import_publish_queue (
      ...
      'Public Pharmacy noindex authority; routes and rollback remain separately locked.',
      jsonb_build_object(
        'public_noindex_schema_version', p_schema_version,
        'public_noindex_authorization_id', v_authorization.id,
        'import_entity_candidate_id', v_candidate.id,
  ...
  v_terminal := jsonb_build_object(
    'kind', 'pharmacy_public_noindex_authority_published',
...
comment on table public.import_pharmacy_public_noindex_authorizations is
  'Protected single-entity Pharmacy public/noindex authority, Queue snapshot and publish readback state.';
comment on table public.import_pharmacy_public_noindex_events is
  'Append-only events for Pharmacy public/noindex authorization and publication.';
