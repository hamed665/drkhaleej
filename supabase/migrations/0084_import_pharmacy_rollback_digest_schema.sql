-- P06 ROLLBACK-AUTHORITY-HARDENING: schema-qualify pgcrypto digest calls.
-- Forward-only correction after isolated Preview proved pgcrypto is installed in the extensions schema.
...
begin
  ...
  where r.actor_profile_id = p_actor_profile_id
    ...
    and r.consumed_result_hash = encode(extensions.digest(r.consumed_result::text, 'sha256'), 'hex')
  ...
  v_consumed_result_hash := encode(extensions.digest(v_consumed_result::text, 'sha256'), 'hex');
...
comment on function public.import_rollback_pharmacy_private_by_authority(
  uuid, uuid, text
) is 'Atomically selects and consumes one server-only Pharmacy rollback authority using schema-qualified pgcrypto hashing, the existing rollback RPC, and bounded replay-safe readback.';
